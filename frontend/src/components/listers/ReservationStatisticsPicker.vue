...
<script>
    ...
    export default {
        name: 'StatisticsReservationStatisticsPicker',
        props: {
        ...
        async created() {
            ...
            if(temp.data) {
                me.list = temp.data._embedded.reservationStatistics;
            }

            if(me.value && typeof me.value == "object" && Object.values(me.value)[0]) {
                var id = Object.values(me.value)[0];
                var tmpValue = await axios.get(axios.fixUrl('/reservationStatistics/' + id))
                if(tmpValue.data) {
